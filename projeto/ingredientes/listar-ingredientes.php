...
<body>
    <div class="container">
<h1 class="text-center" style="margin-bottom: 50px;">INGREDIENTES</h1>
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 50px;">
    <h3>Lista de Ingredientes</h3>
    <button class="btn btn-add-user" style="background-color: #00FF7F;" onclick="window.location.href='?page=novo-ingrediente';">Adicionar</button>
</div>
...
if ($qtd > 0) {
    ...
    while ($row = $res->fetch_object()) {
        ...
        print "<td style='display: flex; justify-content: space-between; align-items: center;'>
                <a href='?page=editar-ingrediente&id=".$row->id."'>".$row->nome."</a>
                <button onclick=\"if(confirm('Tem certeza que deseja excluir?')){location.href='?page=salvar-ingredientes&acao=excluir&id=".$row->id."';}else{return false;}\" class='btn btn-danger'>Excluir</button>
               </td>";
        print "</tr>";
    }
    print "</table>";
